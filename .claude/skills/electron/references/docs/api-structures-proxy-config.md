...
+ `auto_detect` - In auto\_detect mode the proxy configuration is determined by a PAC script that can
be downloaded at <http://wpad/wpad.dat>.
+ `pac_script` - In pac\_script mode the proxy configuration is determined by a PAC script that is
retrieved from the URL specified in the `pacScript`. This is the default mode if `pacScript` is specified.
+ `fixed_servers` - In fixed\_servers mode the proxy configuration is specified in `proxyRules`.
...
"[https://x.\\*.y.com:99](https://x.%5C*.y.com:99)"
