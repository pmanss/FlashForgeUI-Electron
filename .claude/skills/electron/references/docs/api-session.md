...
property of [`WebContents`](/docs/latest/api/web-contents), or from the `session` module.
...
[`--disable-http-cache` switch](/docs/latest/api/command-line-switches#--disable-http-cache) is used.
...
+ `frame` [WebFrameMain](/docs/latest/api/web-frame-main) | null - The frame initiating this event.
...
* `options` Object (optional)
+ `origin` string (optional) - Should follow `window.location.origin`âs representation
`scheme://host:port`.
...
See also [`net.fetch()`](/docs/latest/api/net#netfetchinput-init), a convenience method which
...
By default, requests made with `net.fetch` can be made to [custom protocols](/docs/latest/api/protocol)
as well as `file:`, and will trigger [webRequest](/docs/latest/api/web-request) handlers if present.
...
intercepted request to the built-in handler. [webRequest](/docs/latest/api/web-request)
...
* `handler` Function | null
+ `request` Object
- `frame` [WebFrameMain](/docs/latest/api/web-frame-main) | null - Frame that is requesting access to media.
May be `null` if accessed after the frame has either navigated or been destroyed.
...
come from a [DesktopCapturerSource](/docs/latest/api/structures/desktop-capturer-source)
object.
+ `name` String - The name of the stream being granted. This will
usually come from a [DesktopCapturerSource](/docs/latest/api/structures/desktop-capturer-source)
object.
* `audio` String | [WebFrameMain](/docs/latest/api/web-frame-main) (optional) - If
...
* `enableLocalEcho` Boolean (optional) - If `audio` is a [WebFrameMain](/docs/latest/api/web-frame-main)
...
[desktopCapturer](/docs/latest/api/desktop-capturer) API to choose which stream(s) to grant
access to.
...
Passing a [WebFrameMain](/docs/latest/api/web-frame-main) object as a video or audio stream
...
- `frame` [WebFrameMain](/docs/latest/api/web-frame-main) | null - The frame initiating this handler.
May be `null` if accessed after the frame has either navigated or been destroyed.
...
Ex: To disable TLS\_RSA\_WITH\_RC4\_128\_MD5, specify 0x0004, while to
disable TLS\_ECDH\_ECDSA\_WITH\_RC4\_128\_SHA, specify 0xC002.
...
The API will generate a [DownloadItem](/docs/latest/api/download-item) that can be accessed
with the [will-download](#event-will-download) event.

note

This does not perform any security checks that relate to a page's origin,
unlike [`webContents.downloadURL`](/docs/latest/api/web-contents#contentsdownloadurlurl-options).
...
The API will generate a [DownloadItem](/docs/latest/api/download-item) that can be accessed with the [will-download](#event-will-download)
event. The [DownloadItem](/docs/latest/api/download-item) will not have any `WebContents` associated with it and
the initial state will be `interrupted`. The download will start only when the
`resume` API is called on the [DownloadItem](/docs/latest/api/download-item).
...
[`BrowsingDataRemover`](https://source.chromium.org/chromium/chromium/src/+/main:content/public/browser/browsing_data_remover.h) for the full list.)
...
* [Methods](#methods)
+ [`fromPartition`](#sessionfrompartitionpartition-options)+ [`fromPath`](#sessionfrompathpath-options)* [Properties](#properties)
+ [`defaultSession`](#sessiondefaultsession)* [Class: Session](#class-session)
+ [Instance Events](#instance-events)
- [`'will-download'`](#event-will-download)- [`'extension-loaded'`](#event-extension-loaded)- [`'extension-unloaded'`](#event-extension-unloaded)- [`'extension-ready'`](#event-extension-ready)- [`'file-system-access-restricted'`](#event-file-system-access-restricted)- [`'preconnect'`](#event-preconnect)- [`'spellcheck-dictionary-initialized'`](#event-spellcheck-dictionary-initialized)- [`'spellcheck-dictionary-download-begin'`](#event-spellcheck-dictionary-download-begin)- [`'spellcheck-dictionary-download-success'`](#event-spellcheck-dictionary-download-success)- [`'spellcheck-dictionary-download-failure'`](#event-spellcheck-dictionary-download-failure)- [`'select-hid-device'`](#event-select-hid-device)- [`'hid-device-added'`](#event-hid-device-added)- [`'hid-device-removed'`](#event-hid-device-removed)- [`'hid-device-revoked'`](#event-hid-device-revoked)- [`'select-serial-port'`](#event-select-serial-port)- [`'serial-port-added'`](#event-serial-port-added)- [`'serial-port-removed'`](#event-serial-port-removed)- [`'serial-port-revoked'`](#event-serial-port-revoked)- [`'select-usb-device'`](#event-select-usb-device)- [`'usb-device-added'`](#event-usb-device-added)- [`'usb-device-removed'`](#event-usb-device-removed)- [`'usb-device-revoked'`](#event-usb-device-revoked)+ [Instance Methods](#instance-methods)
- [`getCacheSize`](#sesgetcachesize)- [`clearCache`](#sesclearcache)- [`clearStorageData`](#sesclearstoragedataoptions)- [`flushStorageData`](#sesflushstoragedata)- [`setProxy`](#sessetproxyconfig)- [`resolveHost`](#sesresolvehosthost-options)- [`resolveProxy`](#sesresolveproxyurl)- [`forceReloadProxyConfig`](#sesforcereloadproxyconfig)- [`setDownloadPath`](#sessetdownloadpathpath)- [`enableNetworkEmulation`](#sesenablenetworkemulationoptions)- [`preconnect`](#sespreconnectoptions)- [`closeAllConnections`](#sescloseallconnections)- [`fetch`](#sesfetchinput-init)- [`disableNetworkEmulation`](#sesdisablenetworkemulation)- [`setCertificateVerifyProc`](#sessetcertificateverifyprocproc)- [`setPermissionRequestHandler`](#sessetpermissionrequesthandlerhandler)- [`setPermissionCheckHandler`](#sessetpermissioncheckhandlerhandler)- [`setDisplayMediaRequestHandler`](#sessetdisplaymediarequesthandlerhandler-opts)- [`setDevicePermissionHandler`](#sessetdevicepermissionhandlerhandler)- [`setUSBProtectedClassesHandler`](#sessetusbprotectedclasseshandlerhandler)- [`setBluetoothPairingHandler`](#sessetbluetoothpairinghandlerhandler-windows-linux)- [`clearHostResolverCache`](#sesclearhostresolvercache)- [`allowNTLMCredentialsForDomains`](#sesallowntlmcredentialsfordomainsdomains)- [`setUserAgent`](#sessetuseragentuseragent-acceptlanguages)- [`isPersistent`](#sesispersistent)- [`getUserAgent`](#sesgetuseragent)- [`setSSLConfig`](#sessetsslconfigconfig)- [`getBlobData`](#sesgetblobdataidentifier)- [`downloadURL`](#sesdownloadurlurl-options)- [`createInterruptedDownload`](#sescreateinterrupteddownloadoptions)- [`clearAuthCache`](#sesclearauthcache)- [`setPreloads`](#sessetpreloadspreloads-deprecated)- [`getPreloads`](#sesgetpreloads-deprecated)- [`registerPreloadScript`](#sesregisterpreloadscriptscript)- [`unregisterPreloadScript`](#sesunregisterpreloadscriptid)- [`getPreloadScripts`](#sesgetpreloadscripts)- [`setCodeCachePath`](#sessetcodecachepathpath)- [`clearCodeCaches`](#sesclearcodecachesoptions)- [`getSharedDictionaryUsageInfo`](#sesgetshareddictionaryusageinfo)- [`getSharedDictionaryInfo`](#sesgetshareddictionaryinfooptions)- [`clearSharedDictionaryCache`](#sesclearshareddictionarycache)- [`clearSharedDictionaryCacheForIsolationKey`](#sesclearshareddictionarycacheforisolationkeyoptions)- [`setSpellCheckerEnabled`](#sessetspellcheckerenabledenable)- [`isSpellCheckerEnabled`](#sesisspellcheckerenabled)- [`setSpellCheckerLanguages`](#sessetspellcheckerlanguageslanguages)- [`getSpellCheckerLanguages`](#sesgetspellcheckerlanguages)- [`setSpellCheckerDictionaryDownloadURL`](#sessetspellcheckerdictionarydownloadurlurl)- [`listWordsInSpellCheckerDictionary`](#seslistwordsinspellcheckerdictionary)- [`addWordToSpellCheckerDictionary`](#sesaddwordtospellcheckerdictionaryword)- [`removeWordFromSpellCheckerDictionary`](#sesremovewordfromspellcheckerdictionaryword)- [`loadExtension`](#sesloadextensionpath-options-deprecated)- [`removeExtension`](#sesremoveextensionextensionid-deprecated)- [`getExtension`](#sesgetextensionextensionid-deprecated)- [`getAllExtensions`](#sesgetallextensions-deprecated)- [`getStoragePath`](#sesgetstoragepath)- [`clearData`](#sescleardataoptions)+ [Instance Properties](#instance-properties)
- [`availableSpellCheckerLanguages`](#sesavailablespellcheckerlanguages-readonly)- [`spellCheckerEnabled`](#sesspellcheckerenabled)- [`storagePath`](#sesstoragepath-readonly)- [`cookies`](#sescookies-readonly)- [`extensions`](#sesextensions-readonly)- [`serviceWorkers`](#sesserviceworkers-readonly)- [`webRequest`](#seswebrequest-readonly)- [`protocol`](#sesprotocol-readonly)- [`netLog`](#sesnetlog-readonly)