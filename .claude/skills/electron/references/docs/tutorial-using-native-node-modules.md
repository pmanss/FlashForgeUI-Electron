...
[application binary interface (ABI)](https://en.wikipedia.org/wiki/Application_binary_interface) from a given Node.js binary (due to
...
for Electron with the [`@electron/rebuild`](https://github.com/electron/rebuild) package. This
...
If you are using [Electron Forge](https://electronforge.io/), this tool is used automatically
...
[Electron Packager](https://github.com/electron/packager), consult the project's README.
...
The [`node-pre-gyp` tool](https://github.com/mapbox/node-pre-gyp) provides a way to deploy native Node
...
+ [Installing modules and rebuilding for Electron](#installing-modules-and-rebuilding-for-electron)+ [Using `npm`](#using-npm)+ [Manually building for Electron](#manually-building-for-electron)+ [Manually building for a custom build of Electron](#manually-building-for-a-custom-build-of-electron)* [Troubleshooting](#troubleshooting)
+ [A note about `win_delay_load_hook`](#a-note-about-win_delay_load_hook)* [Modules that rely on `prebuild`](#modules-that-rely-on-prebuild)* [Modules that rely on `node-pre-gyp`](#modules-that-rely-on-node-pre-gyp)