...
with the [`ipcMain`](/docs/latest/api/ipc-main) and [`ipcRenderer`](/docs/latest/api/ipc-renderer) modules. These channels are
...
[preload script](/docs/latest/tutorial/process-model#preload-scripts) to import Node.js and Electron modules in a context-isolated renderer process.
...
out the [context isolation tutorial](/docs/latest/tutorial/context-isolation).
...
[`ipcRenderer.send`](/docs/latest/api/ipc-renderer) API to send a message that is then received by the [`ipcMain.on`](/docs/latest/api/ipc-main) API.
...
[docs/fiddles/ipc/pattern-1 (40.0.0)](https://github.com/electron/electron/tree/v40.0.0/docs/fiddles/ipc/pattern-1)[Open in Fiddle](https://fiddle.electronjs.org/launch?target=electron/v40.0.0/docs/fiddles/ipc/pattern-1)
...
The above `handleSetTitle` callback has two parameters: an [IpcMainEvent](/docs/latest/api/structures/ipc-main-event) structure and a
...
We don't directly expose the whole `ipcRenderer.send` API for [security reasons](/docs/latest/tutorial/context-isolation#security-considerations). Make sure to
...
code and waiting for a result. This can be done by using [`ipcRenderer.invoke`](/docs/latest/api/ipc-renderer#ipcrendererinvokechannel-args) paired with
[`ipcMain.handle`](/docs/latest/api/ipc-main#ipcmainhandlechannel-listener).
...
[docs/fiddles/ipc/pattern-2 (40.0.0)](https://github.com/electron/electron/tree/v40.0.0/docs/fiddles/ipc/pattern-2)[Open in Fiddle](https://fiddle.electronjs.org/launch?target=electron/v40.0.0/docs/fiddles/ipc/pattern-2)
...
We don't directly expose the whole `ipcRenderer.invoke` API for [security reasons](/docs/latest/tutorial/context-isolation#security-considerations). Make sure to
limit the renderer's access to Electron APIs as much as possible.
...
via its [`WebContents`](/docs/latest/api/web-contents) instance. This WebContents instance contains a [`send`](/docs/latest/api/web-contents#contentssendchannel-args) method
...
[docs/fiddles/ipc/pattern-3 (40.0.0)](https://github.com/electron/electron/tree/v40.0.0/docs/fiddles/ipc/pattern-3)[Open in Fiddle](https://fiddle.electronjs.org/launch?target=electron/v40.0.0/docs/fiddles/ipc/pattern-3)
...
We don't directly expose the whole `ipcRenderer.on` API for [security reasons](/docs/latest/tutorial/context-isolation#security-considerations). Make sure to
limit the renderer's access to Electron APIs as much as possible.
...
* Pass a [MessagePort](/docs/latest/tutorial/message-ports) from the main process to both renderers. This will allow direct communication
...
[Structured Clone Algorithm](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm) to serialize objects passed between processes, meaning that
...
* [IPC channels](#ipc-channels)* [Understanding context-isolated processes](#understanding-context-isolated-processes)* [Pattern 1: Renderer to main (one-way)](#pattern-1-renderer-to-main-one-way)
+ [1. Listen for events with `ipcMain.on`](#1-listen-for-events-with-ipcmainon)+ [2. Expose `ipcRenderer.send` via preload](#2-expose-ipcrenderersend-via-preload)+ [3. Build the renderer process UI](#3-build-the-renderer-process-ui)* [Pattern 2: Renderer to main (two-way)](#pattern-2-renderer-to-main-two-way)
+ [1. Listen for events with `ipcMain.handle`](#1-listen-for-events-with-ipcmainhandle)+ [2. Expose `ipcRenderer.invoke` via preload](#2-expose-ipcrendererinvoke-via-preload)+ [3. Build the renderer process UI](#3-build-the-renderer-process-ui-1)+ [Note: legacy approaches](#note-legacy-approaches)
- [Using `ipcRenderer.send`](#using-ipcrenderersend)- [Using `ipcRenderer.sendSync`](#using-ipcrenderersendsync)* [Pattern 3: Main to renderer](#pattern-3-main-to-renderer)
+ [1. Send messages with the `webContents` module](#1-send-messages-with-the-webcontents-module)+ [2. Expose `ipcRenderer.on` via preload](#2-expose-ipcrendereron-via-preload)+ [3. Build the renderer process UI](#3-build-the-renderer-process-ui-2)+ [Optional: returning a reply](#optional-returning-a-reply)* [Pattern 4: Renderer to renderer](#pattern-4-renderer-to-renderer)* [Object serialization](#object-serialization)