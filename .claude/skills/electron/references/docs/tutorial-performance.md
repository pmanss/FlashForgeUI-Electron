...
This list is provided for your convenience â and is, much like our
[security checklist](/docs/latest/tutorial/security) â not meant to be exhaustive. It is probably possible
...
the GPU process about that â once again going through the main process.
...
[worker threads](https://nodejs.org/api/worker_threads.html), consider moving them to the BrowserWindow, or
...
[check out its documentation on MDN](https://developer.mozilla.org/en-US/docs/Web/API/Window/requestIdleCallback).

*Web Workers* are a powerful tool to run code on a separate thread. There are
some caveats to consider âÂ consult Electron's
[multithreading documentation](/docs/latest/tutorial/multithreading) and the
[MDN documentation for Web Workers](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers). They're an ideal solution
...
and check if the [version of Chromium used in your Electron version](/docs/latest/api/process#processversionschrome-readonly)
...
of the [standard JavaScript feature set available](https://youmightnotneedjquery.com/).
...
As of writing this article, the popular choices include [Webpack](https://webpack.js.org/),
[Parcel](https://parceljs.org/), and [rollup.js](https://rollupjs.org/).
...
+ [Recommended Reading](#recommended-reading)* [Checklist: Performance recommendations](#checklist-performance-recommendations)
+ [1. Carelessly including modules](#1-carelessly-including-modules)+ [2. Loading and running code too soon](#2-loading-and-running-code-too-soon)+ [3. Blocking the main process](#3-blocking-the-main-process)+ [4. Blocking the renderer process](#4-blocking-the-renderer-process)+ [5. Unnecessary polyfills](#5-unnecessary-polyfills)+ [6. Unnecessary or blocking network requests](#6-unnecessary-or-blocking-network-requests)+ [7. Bundle your code](#7-bundle-your-code)+ [8. Call `Menu.setApplicationMenu(null)` when you do not need a default menu](#8-call-menusetapplicationmenunull-when-you-do-not-need-a-default-menu)